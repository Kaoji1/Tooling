-- =============================================
-- SP GM WITH 5-COLUMN SAFETY CHECK (FIXED)
-- PartNo, ItemNo, Process, MC, Spec
-- =============================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- ==========================================================
        -- SAFETY: ลบ Duplicate ใน Staging ก่อน (5 columns)
        -- ==========================================================
        ;WITH CTE_Dup AS (
            SELECT *, ROW_NUMBER() OVER(
                PARTITION BY PartNo, ItemNo, Process, MC, Spec
                ORDER BY StagingID
            ) as rn
        ...
        -- ==========================================================
        -- PART 3: INSERT CUTTING (5 columns check via Spec name)
        -- ==========================================================
        ...
        AND NOT EXISTS (
            ...
            WHERE T.PartNo = STG.PartNo 
              AND T.ItemNo = STG.ItemNo 
              AND T.Process = STG.Process 
              AND T.MC = STG.MC
        );
