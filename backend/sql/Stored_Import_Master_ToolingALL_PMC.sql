...
-- Date:           สร้างตอน 26/1/2569  อัพเดท ตอน 3/2/2569
-- Description:    Stored_Import_Master_ToolingALL_PMC
-- FIXED:          เพิ่ม Output Parameters สำหรับ row counts
-- =============================================
ALTER PROCEDURE [trans].[Stored_Import_Master_ToolingALL_PMC]
    @BatchID UNIQUEIDENTIFIER,
    @CuttingUpdated INT OUTPUT,
    @CuttingInserted INT OUTPUT,
    @SetupUpdated INT OUTPUT,
    @SetupInserted INT OUTPUT
AS
BEGIN
    SET NOCOUNT ON;
    
    -- Initialize output
    SET @CuttingUpdated = 0;
    SET @CuttingInserted = 0;
    SET @SetupUpdated = 0;
    SET @SetupInserted = 0;
