-- =======================================================
-- Update Stored_PCPlan_Query to include ReqStatus
-- from the linked request document tables (via Unique_Id).
--
-- ReqStatus is the raw Status populated in:
...
BEGIN
    ...
    BEGIN
        ...
        SELECT
            ...
            END AS Status_To_PH,
            -- Merge ReqStatus: Status from the Request tables
            CASE
                WHEN ISNULL(CT.[ReqStatus], '') = 'Waiting' OR ISNULL(ST.[ReqStatus], '') = 'Waiting' 
                     OR ISNULL(CT.[ReqStatus], '') = 'In Progress' OR ISNULL(ST.[ReqStatus], '') = 'In Progress'
                    THEN 'Waiting' -- Simplify to Waiting/In Progress for the frontend tab
                ELSE COALESCE(CT.[ReqStatus], ST.[ReqStatus], NULL)
            END AS ReqStatus
        FROM [master].[tb_PC_Plan] Main
        LEFT JOIN (
            SELECT [Unique_Id], MAX([Status_To_PH]) AS [Status_To_PH], MAX([Status]) AS [ReqStatus]
