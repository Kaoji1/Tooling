...
-- =============================================
-- Updated: Stored_Get_CaseSET_All
-- แก้ไข: แสดง Item ทั้งหมดใน PartNo โดยไม่กรองตาม Facility
--        แต่ยังแสดงจำนวน FreshQty/ReuseQty ตาม Facility ที่เลือก
-- =============================================
ALTER PROCEDURE [trans].[Stored_Get_CaseSET_All]
...
BEGIN
    ...
    -- 1. Pre-aggregate Stock Qty based on Facility
    --    ยังคงกรองตาม Facility เพื่อแสดงจำนวนที่ถูกต้อง
    ;WITH Stock_Agg AS (
        SELECT 
            ToolingName, 
            PC, 
    ...
    --    ไม่กรอง Item ตาม Facility แล้ว → แสดง Item ทั้งหมดใน PartNo
    --    ถ้า Item ไม่มี Stock ใน Facility → FreshQty=0, ReuseQty=0
    SELECT DISTINCT
        M.*,
        M.Part_No AS PartNo,
    ...
    WHERE (@Input_Division IS NULL OR M.[Profit_Center] = @Input_Division)
      ...
      -- ลบ EXISTS Facility filter ออก → แสดง Item ทั้งหมดไม่ว่าจะมี Stock ใน Facility หรือไม่
