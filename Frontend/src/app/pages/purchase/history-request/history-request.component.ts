...
exportexcel() {
  ...
  // เลือก thead ชุดล่างที่เป็น <th> จริง (class="table-light")
  const thead = table.querySelector("thead.table-light") as HTMLTableSectionElement;
  const tbody = table.querySelector("tbody");
  ...
  // clone thead และลบ column ที่ไม่ต้องการ
if (thead) {
  const clonedThead = thead.cloneNode(true) as HTMLTableSectionElement;

  // ลบ <th> ตาม index เดียวกับ td
  clonedThead.querySelectorAll("th").forEach((th, index) => {
    if (["0", "1", "2", "10","14", "15", "16"].includes(index.toString())) {
      th.remove();
    }
  });
  ...
  selectedRows.forEach(row => {
    ...
    // ลบ column ที่ไม่ต้องการ (เช่น checkbox, edit button)
    clonedRow.querySelectorAll("td").forEach((td, index) => {
      if (["0", "1", "2", "10","14", "15", "16"].includes(index.toString())) {
        td.remove();
      }
